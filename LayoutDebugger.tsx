...
export const LayoutDebugger: React.FC = () => {
  ...
  const analyzePage = useCallback(() => {
    ...
    if (!document.documentElement.hasAttribute('lang') || !document.documentElement.getAttribute('lang')?.trim()) {
      newErrors.push({ id: 'html-lang', type: 'html', text: t.noLang, el: document.documentElement });
    }

    if (document.querySelectorAll('h1').length === 0) {
      newErrors.push({ id: 'html-h1', type: 'html', text: t.noH1, el: document.body });
    }
    ...
    elements.forEach((el, index) => {
      ...
      if (htmlEl.tagName === 'IMG' && (!htmlEl.hasAttribute('alt') || !htmlEl.getAttribute('alt')?.trim())) {
